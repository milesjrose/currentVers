...
class TestComparativeSemantics:
    ...
    def test_init_comparative_semantics_empty(self):
        ...
        for sdm in SDM:
            assert semantics.sdms[sdm] is None
        ...
        for sdm in SDM:
            assert semantics.sdms[sdm] is not None
        
        # Should be Ref_Semantic objects
        for sdm in SDM:
            assert isinstance(semantics.sdms[sdm], Ref_Semantic)
        
        # Should be added to names
        for sdm in SDM:
            assert sdm.name in semantics.names.values()
    ...
    def test_init_comparative_semantics_existing_names(self):
        ...
        connections = torch.zeros(3, 3)
        IDs = {1: 0, 2: 1, 3: 2}
        names = {1: "test1", 2: SDM.MORE.name, 3: "test3"}  # "more" already exists
        
        semantics = Semantics(nodes, connections, IDs, names)
        semantics.init_sdm()
        
        # Should not create new "more" semantic
        assert semantics.sdms[SDM.MORE] is None
        # But should create "less" and "same"
        assert semantics.sdms[SDM.LESS] is not None
        assert semantics.sdms[SDM.SAME] is not None
        
        # Should be Ref_Semantic objects
        for sdm in [SDM.LESS, SDM.SAME]:
            assert hasattr(semantics.sdms[sdm], 'ID')
            assert hasattr(semantics.sdms[sdm], 'name')
